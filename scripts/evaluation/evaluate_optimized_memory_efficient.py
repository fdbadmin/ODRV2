#!/usr/bin/env python3
"""
Memory-Efficient Optimized Ensemble Evaluation

Loads one fold model at a time to reduce memory usage.
"""
...
def get_fold_predictions(fold_idx, dataloader, threshold):
    ...
    Loads model, gets predictions, then unloads to save memory.
    """
...
def main():
    print("="*80)
    print("MEMORY-EFFICIENT OPTIMIZED ENSEMBLE EVALUATION")
    print("="*80)
    ...
    print_section("Processing Folds (One at a Time)")
    fold_predictions_list = []
    
    start_time = time.time()
    
    for fold_idx in range(5):
        fold_preds = get_fold_predictions(fold_idx, dataloader, fold_thresholds[fold_idx])
        fold_predictions_list.append(fold_preds)
        
        elapsed = time.time() - start_time
        remaining_folds = 5 - (fold_idx + 1)
        est_remaining = (elapsed / (fold_idx + 1)) * remaining_folds
    ...
    for fold_idx in range(5):
        weighted_predictions += fold_weights[fold_idx] * fold_predictions_list[fold_idx]
    ...
    print(f"Optimized (Per-Fold + Weighted):       {macro_f1:.4f}")
    ...
    np.save(OUTPUT_DIR / "optimized_predictions.npy", final_predictions)
